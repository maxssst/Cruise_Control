...
if (isset($_POST["btn_submit_register"]) && !empty($_POST["btn_submit_register"])) {
    ...
    if (isset($_POST["captcha"]) && !empty($captcha)) {
        ...
        $result_query_insert = $mysqli->query("INSERT INTO `users` (`id`, `first_name`, `last_name`, `email`, `email_sattus`, `password`) VALUES (NULL, '" . $first_name . "', '" . $last_name . "', '" . $email . "', '0', '" . $password . "')");
        ...
        if (!$result_query_insert) {
            ...
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: " . $address_site . "/form_register.php");
            //header("Location: http://sender/index/form_register.php");
            ...
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: http://sender/form_auth.php");
            //Останавливаем скрипт
            exit();
        }
    ...
    <div class=\"col-md-6\">
<div class=\"h-100 p-6 bg-light border rounded-3\">
<br><br>
                <h2>No Access</h2>
                <strong>Ошибка!</strong> 
                <p>
    Вы зашли на эту страницу напрямую, поэтому нет данных для обработки. Вы можете перейти на 
                 </p>
                <div class=\"mbr-section-btn\">
                    <div id=\"link_register\">
                        <a class=\"btn btn-primary display-4\" href=\"/form_register.php\">Sign Up</a>
                    </div>

                    <div id=\"link_auth\">
                        <a class=\"btn btn-primary display-4\" href=\"/form_auth.php\">Log In</a>
                    </div>
                </div>
            </div>
        </div>
